...
def check_treatment_missing_info(t: dict) -> str:
    """দৈনিক Treatment এন্ট্রি সেভ হওয়ার ঠিক আগে চেক করে — থেরাপিস্ট ইতিমধ্যে লিখে দেওয়া
    রোগীর মন্তব্য (t["Remarks"]) বাদ দিয়ে, ক্লিনিক্যালি সত্যিই গুরুত্বপূর্ণ/Safety-critical
    কোনো তথ্য এখনো মিসিং কিনা AI যাচাই করে। দরকার হলে ১টা ছোট, নির্দিষ্ট প্রশ্ন (str) রিটার্ন
    করে; রোগীর মন্তব্যে যা আগেই বলা হয়ে গেছে বা রুটিন কিছুতে কখনো প্রশ্ন করে না।"""
    if not OPENROUTER_API_KEY:
        return ""
    ...
    note_text = "\n".join(note_lines)
    patient_remarks = t.get("Remarks", "") or "(থেরাপিস্ট আজ কোনো মন্তব্য লেখেনি)"

    system_msg = (
        "তুমি একজন Senior Physiotherapy Clinical Reviewer। নিচে আজকের একটা রুটিন ট্রিটমেন্ট "
        "সেশনের সংক্ষিপ্ত নোট এবং থেরাপিস্ট ইতিমধ্যে লিখে দেওয়া রোগীর মন্তব্য দেওয়া হলো। "
        "এই দুটো পড়ে দেখো — ক্লিনিক্যালি সত্যিই গুরুত্বপূর্ণ কোনো তথ্য (বিশেষ করে Safety/Red-Flag "
        "সংক্রান্ত — যেমন ব্যথা হঠাৎ তীব্র বেড়ে যাওয়া, নতুন নিউরোলজিক্যাল উপসর্গ, পোস্ট-অপ কমপ্লিকেশন "
        "সাইন, জ্বর, অস্বাভাবিক ফোলা/লালচেভাব) এখনো সত্যিই মিসিং কিনা। "
        "রোগীর মন্তব্যে যা ইতিমধ্যে বলা হয়ে গেছে তা নিয়ে কখনো আবার প্রশ্ন কোরো না — শুধু যেটা সত্যিই "
        "অজানা/বাকি থেকে গেছে সেটা নিয়েই প্রশ্ন করবে। রুটিন কিছুতে (এক্সারসাইজ কেমন হলো, সাধারণ "
        "প্রোগ্রেস ইত্যাদি সাধারণ ব্যাপারে) কখনো প্রশ্ন কোরো না — শুধু আসল প্রয়োজন থাকলেই একটা মাত্র "
        "ছোট, নির্দিষ্ট প্রশ্ন লেখো। "
        "উত্তর অবশ্যই শুধু plain text-এ ১টা প্রশ্ন হবে, JSON বা markdown ছাড়া। প্রশ্নের দরকার না "
        "থাকলে শুধু 'NONE' লিখো, অন্য কিছু না।"
    )
    user_msg = f"আজকের ট্রিটমেন্ট নোট:\n{note_text}\n\nথেরাপিস্টের লেখা রোগীর মন্তব্য:\n{patient_remarks}"
    ...
    try:
        ...
        print(f"[case_study_ai] check_treatment_missing_info FAILED: {e}")
        return ""
