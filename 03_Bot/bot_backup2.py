...
async def bug_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "🐛 কী সমস্যা হচ্ছে লিখো (এক বা একাধিক লাইনে):",
        reply_markup=ReplyKeyboardRemove(),
    )
    return BUG_DESC


async def bug_desc(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data["bug_report"]["Description"] = update.message.text.strip()
    await update.message.reply_text(
        "স্ক্রিনশট/ছবি থাকলে পাঠাও, না থাকলে - দাও:",
        reply_markup=_skip_keyboard(),
    )
    return BUG_PHOTO


async def bug_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    staff = context.user_data.get("staff", {})
    photo_file_id = ""
    if update.message.photo:
        photo_file_id = update.message.photo[-1].file_id
    bug = context.user_data.get("bug_report", {})
    bug["Reported_By"] = staff.get("Full_Name", "Unknown")
    bug["Role"] = staff.get("Role", "")
    bug["Photo_File_ID"] = photo_file_id
    bug_id = sheets.add_bug_report(bug)

    await update.message.reply_text(
        f"✅ ধন্যবাদ, সমস্যা রিপোর্ট করা হয়েছে। (ID: {bug_id})",
        reply_markup=_menu_keyboard(staff.get("Role", "")),
    )

    owner_ids = sheets.get_staff_telegram_ids_by_role("Owner")
    alert_text = (
        f"🐛 নতুন সমস্যা রিপোর্ট (ID: {bug_id})\n"
        f"রিপোর্ট করেছেন: {bug['Reported_By']} ({bug['Role']})\n\n"
        f"{bug['Description']}"
    )
        ...
        try:
            if photo_file_id:
                await context.bot.send_photo(
                    chat_id=owner_id, photo=photo_file_id, caption=alert_text
                )
            else:
                await context.bot.send_message(chat_id=owner_id, text=alert_text)
        except Exception:
            logger.exception("Owner-কে বাগ রিপোর্ট নোটিফাই করতে ব্যর্থ হয়েছে")
...
def _bug_list_keyboard(bug_id: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [[InlineKeyboardButton("✅ সমাধান হয়েছে", callback_data=f"bugfix_{bug_id}")]]
    )


async def bug_list_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    staff = context.user_data.get("staff") or await _require_staff(update, context)
    if staff is None:
        return
    if not roles.can_access(staff.get("Role", ""), roles.MENU_BUG_LIST):
        await update.message.reply_text("⛔ এই মেনুতে তোমার অনুমতি নেই।")
        return
    open_bugs = sheets.get_open_bug_reports()
    if not open_bugs:
        await update.message.reply_text("✅ কোনো Open সমস্যা নেই।")
        return
    ...
    for bug in open_bugs:
        ...
        photo_id = bug.get("Photo_File_ID", "")
        keyboard = _bug_list_keyboard(bug.get("Bug_ID", ""))
        if photo_id:
...
async def bug_fix_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    bug_id = query.data.replace("bugfix_", "")
    sheets.mark_bug_report_fixed(bug_id)
    try:
        await query.edit_message_reply_markup(reply_markup=None)
    except Exception:
        pass
    await query.message.reply_text(f"✅ {bug_id} সমাধান হয়েছে বলে মার্ক করা হলো।")
...
def main():
    ...
    app.add_handler(
        MessageHandler(filters.Regex(f"^{roles.MENU_BUG_LIST}$"), bug_list_menu)
    )
    app.add_handler(CallbackQueryHandler(bug_fix_callback, pattern="^bugfix_"))

    bug_conv = ConversationHandler(
        entry_points=[
            MessageHandler(filters.Regex(f"^{roles.MENU_BUG_REPORT}$"), bug_start)
        ],
        states={
            BUG_DESC: [MessageHandler(filters.TEXT & ~filters.COMMAND & ~filters.Regex(_ALL_MENU_REGEX), bug_desc)],
            BUG_PHOTO: [
                MessageHandler(filters.PHOTO, bug_photo),
                MessageHandler(filters.TEXT & ~filters.COMMAND & ~filters.Regex(_ALL_MENU_REGEX), bug_photo),
            ],
        },
        fallbacks=[
            MessageHandler(filters.Regex(_ALL_MENU_REGEX), _cancel_on_menu_press),
