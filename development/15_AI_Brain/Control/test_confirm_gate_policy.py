...
class ConfirmGatePolicyTests(unittest.TestCase):
    ...
    def test_outputs_are_low_risk(self):
        decision = self.gate.classify_target("development/15_AI_Brain/Outputs/TASK-TEST.md")
    ...
    def test_control_source_requires_manual_review(self):
        decision = self.gate.classify_target("development/15_AI_Brain/Control/scheduler.py")
    ...
    def test_path_traversal_cannot_disguise_production_write(self):
        decision = self.gate.classify_target("development/15_AI_Brain/Outputs/../../../03_Bot/bot.py")
        self.assertEqual(decision["decision"], "BLOCKED_PRODUCTION")
    ...
    def test_safe_apply_writes_only_output_path(self):
        result = self.gate.safe_auto_apply(
            "TASK-TEST",
            "development/15_AI_Brain/Outputs/TASK-TEST.md",
            "health check ok\n",
        )
        self.assertTrue(result["applied"])
        target = os.path.join(self.tmp.name, "development/15_AI_Brain", "Outputs", "TASK-TEST.md")
    ...
    def test_safe_apply_refuses_path_traversal_into_production(self):
        result = self.gate.safe_auto_apply(
            "TASK-ESCAPE",
            "development/15_AI_Brain/Outputs/../../../03_Bot/bot.py",
            "unsafe",
        )
        self.assertFalse(result["applied"])
        self.assertEqual(result["decision"], "BLOCKED_PRODUCTION")
    ...
    def test_symlink_escape_is_blocked_even_though_prefix_matches(self):
        """development/15_AI_Brain/Outputs/ is replaced with a symlink pointing at a
        ...
        brain_dir = Path(self.tmp.name) / "development/15_AI_Brain"
        ...
        result = self.gate.safe_auto_apply(
            "TASK-SYMLINK-ESCAPE",
            "development/15_AI_Brain/Outputs/escape.md",
