...
def get_payments_for_date(date_str: str) -> list[dict]:
    """একটা নির্দিষ্ট তারিখের সব পেমেন্ট এন্ট্রি, SL অনুযায়ী সাজানো —
    'আজকের রেজিস্টার' রিপোর্টের জন্য ব্যবহৃত হয়।"""
    all_payments = get_all_payments()
    todays = [p for p in all_payments if str(p.get("Date", "")).strip() == date_str.strip()]
    todays.sort(key=lambda p: int(p.get("SL", 0) or 0))
    return todays
