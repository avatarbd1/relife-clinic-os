...
রান করার নিয়ম:
  cd ~/relife-clinic-os
  python3 15_AI_Brain/Monitor/self_healing.py
...
REPORT_DIR = PROJECT_ROOT / "15_AI_Brain" / "Monitor"
...
def try_provider_router_healthcheck(project_root):
    core_path = project_root / "15_AI_Brain" / "Core"
    if not core_path.exists():
        return {"status": "skipped", "reason": "15_AI_Brain/Core not found"}

    sys.path.insert(0, str(core_path))
    sys.path.insert(0, str(project_root / "15_AI_Brain" / "Control"))
    try:
